          ...
          <tr>
            <td style="padding:36px 32px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
                <tr>
                  <td style="padding:8px 0;border-bottom:1px solid #1e1e2e;">
                    ...
                    <p style="margin:4px 0 0;font-size:15px;color:#e0e0ff;font-weight:600;">${escapeHtml(name)}</p>
                  </td>
                </tr>
                <tr>
                  <td style="padding:8px 0;border-bottom:1px solid #1e1e2e;">
                    <span style="font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">Email</span>
                    <p style="margin:4px 0 0;font-size:15px;color:#a5b4fc;">${escapeHtml(email)}</p>
                  </td>
                </tr>
                <tr>
                  <td style="padding:8px 0;border-bottom:1px solid #1e1e2e;">
                    <span style="font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">Purpose</span>
                    <p style="margin:4px 0 0;font-size:15px;color:#e0e0ff;font-weight:600;text-transform:capitalize;">${escapeHtml(purpose || 'Not specified')}</p>
                  </td>
                </tr>
              </table>
              ...
              <div style="background:#1a1a2e;border:1px solid #6366f1;border-radius:10px;padding:18px 22px;margin-bottom:20px;">
                ...
                <p style="margin:0;font-size:14px;color:#d1d5db;line-height:1.6;white-space:pre-wrap;">${escapeHtml(message)}</p>
              </div>

              <p style="margin:0;color:#6b7280;font-size:12px;">Reply directly to this email to respond to ${escapeHtml(name)}.</p>
            </td>
          </tr>
          <tr>
        ...
        <table width="520" cellpadding="0" cellspacing="0" style="background:#11111a;border:1px solid #1e1e2e;border-radius:12px;overflow:hidden;">
          ...
          <tr>
            <td style="padding:36px 32px;text-align:center;">
              <p style="margin:0 0 8px;color:#9ca3af;font-size:14px;">Hi <strong style="color:#e0e0ff;">${escapeHtml(name)}</strong>,</p>
              ...
              <div style="display:inline-block;background:#1a1a2e;border:2px solid #6366f1;border-radius:10px;padding:18px 36px;margin-bottom:28px;">
                <span style="font-size:13px;font-weight:600;letter-spacing:1px;color:#a5b4fc;">Your message has been received.</span>
              </div>
...
router.post('/send', contactLimiter, async (req, res) => {
  try {
    ...
    if (!OWNER_EMAIL) {
      console.error('[contact] OWNER_EMAIL is not configured.');
      return res.status(500).json({ error: 'Contact notifications are not configured on the server.' });
    }
    ...
    return res.status(200).json({
      message: 'Your message has been sent. Watch your inbox for a confirmation.',
    });
  } catch (err) {
