...
const buildOwnerNotificationHtml = ({
  reporterName,
  reporterEmail,
  repoName,
  threadTitle,
      ...
      <table width="540" cellpadding="0" cellspacing="0" style="background:#11111a;border:1px solid #1e1e2e;border-radius:12px;overflow:hidden;">
        ...
        <tr>
          <td style="padding:32px;">
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
              <tr>
                <td style="padding:8px 0;border-bottom:1px solid #1e1e2e;">
                  ...
                  <p style="margin:4px 0 0;font-size:15px;color:#e0e0ff;font-weight:600;">${escapeHtml(reporterName)}</p>
                </td>
              </tr>
              <tr>
                <td style="padding:8px 0;border-bottom:1px solid #1e1e2e;">
                  <span style="font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">Email</span>
                  <p style="margin:4px 0 0;font-size:15px;color:#a5b4fc;">${escapeHtml(reporterEmail)}</p>
                </td>
              </tr>
              <tr>
                <td style="padding:8px 0;border-bottom:1px solid #1e1e2e;">
                  <span style="font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">Repository</span>
                  <p style="margin:4px 0 0;font-size:15px;color:#e0e0ff;">${escapeHtml(repoName || '-')}</p>
                </td>
              </tr>
              <tr>
                <td style="padding:8px 0;border-bottom:1px solid #1e1e2e;">
                  <span style="font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">Conversation</span>
                  <p style="margin:4px 0 0;font-size:15px;color:#e0e0ff;">${escapeHtml(threadTitle || 'Unnamed conversation')}</p>
                </td>
              </tr>
              <tr>
                <td style="padding:8px 0;">
                  <span style="font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">Reported At</span>
                  <p style="margin:4px 0 0;font-size:14px;color:#9ca3af;">${escapeHtml(reportedAt)}</p>
                </td>
              </tr>
            </table>
            ...
            <div style="background:#1a1a2e;border-left:4px solid #ef4444;border-radius:0 8px 8px 0;padding:16px 20px;margin-bottom:20px;">
              <p style="margin:0;font-size:13px;color:#d1d5db;line-height:1.7;white-space:pre-wrap;">${escapeHtml(messageContent)}</p>
            </div>
        ...
        <tr>
          <td style="padding:16px 32px;border-top:1px solid #1e1e2e;text-align:center;">
            <p style="margin:0;color:#4b5563;font-size:11px;">&copy; ${new Date().getFullYear()} Cyberspace-X. Tron AI Report System.</p>
          </td>
        </tr>
      </table>
    </td></tr>
      ...
      <table width="540" cellpadding="0" cellspacing="0" style="background:#11111a;border:1px solid #1e1e2e;border-radius:12px;overflow:hidden;">
        ...
        <tr>
          <td style="padding:36px 32px;">
            <p style="margin:0 0 6px;font-size:15px;color:#e0e0ff;">Dear <strong>${escapeHtml(reporterName)}</strong>,</p>
...
router.post('/flag', reportLimiter, async (req, res) => {
  try {
    ...
    if (!OWNER_EMAIL) {
      console.error('[report] OWNER_EMAIL is not configured.');
      return res.status(500).json({ error: 'Report notifications are not configured on the server.' });
    }
